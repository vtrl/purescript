...
usedIdents moduleName = ordNub . usedIdents' S.empty . valdeclExpression
  ...
  usedNamesE scope (Var _ (Qualified ByNothing name))
    | LocalIdent name `S.notMember` scope = [name]
  usedNamesE scope (Var _ (Qualified (ByModule moduleName') name))
...
usedImmediateIdents moduleName =
  ...
  usedNamesE True (Var _ (Qualified ByNothing name)) = (True, [name])
  usedNamesE True (Var _ (Qualified (ByModule moduleName') name))
...
usedTypeNames moduleName = go
  ...
  usedNames (TypeConstructor _ (Qualified (ByModule moduleName') name))
    | moduleName == moduleName' = [name]
  usedNames _ = []

  usedConstraint :: SourceConstraint -> [ProperName 'TypeName]
  usedConstraint (Constraint _ (Qualified (ByModule moduleName') name) _ _ _)
...
toDataBindingGroup (CyclicSCC ds')
  ...
  where
  kindDecl (KindDeclaration sa _ pn _) = [(fst sa, Qualified ByNothing pn)]
  kindDecl (ExternDataDeclaration sa pn _) = [(fst sa, Qualified ByNothing pn)]
  kindDecl _ = []
