...
desugarSignedLiterals (Module ss coms mn ds exts) =
  ...
  go (UnaryMinus ss' val) = App (Var ss' (Qualified ByNothing (Ident C.negate))) val
  go other = other
...
removeBinaryNoParens (Parens (stripPositionInfo -> BinaryNoParens op l r))
  | isAnonymousArgument r = do arg <- freshIdent'
                               return $ Abs (VarBinder nullSourceSpan arg) $ App (App op l) (Var nullSourceSpan (Qualified ByNothing arg))
  | isAnonymousArgument l = do arg <- freshIdent'
                               return $ Abs (VarBinder nullSourceSpan arg) $ App (App op (Var nullSourceSpan (Qualified ByNothing arg))) r
...
externsFixities ExternsFile{..} =
  ...
  fromFixity (ExternsFixity assoc prec op name) =
    Left
      ( Qualified (ByModule efModuleName) op
...
collectFixities (Module _ _ moduleName ds _) = concatMap collect ds
  ...
  collect (ValueFixityDeclaration (ss, _) fixity name op) =
    [Left (Qualified (ByModule moduleName) op, ss, fixity, name)]
  collect (TypeFixityDeclaration (ss, _) fixity name op) =
    [Right (Qualified (ByModule moduleName) op, ss, fixity, name)]
  collect _ = []
...
ensureNoDuplicates toError m = go $ sortOn fst m
  ...
  go [] = return ()
  go [_] = return ()
  go ((x@(Qualified (ByModule mn) op), _) : (y, pos) : _) | x == y =
...
checkFixityExports m@(Module ss _ mn ds (Just exps)) =
  ...
  getTypeOpAlias op =
    ...
    where
    go (TypeFixity _ (Qualified (ByModule mn') ident) op')
  ...
  getValueOpAlias op =
    ...
    where
    go (ValueFixity _ (Qualified (ByModule mn') ident) op')
      | mn == mn' && op == op' = Just ident
    go _ = Nothing
