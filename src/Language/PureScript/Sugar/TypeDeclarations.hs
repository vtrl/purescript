...
desugarTypeDeclarationsModule (Module modSS coms name ds exps) =
  ...
  desugarTypeDeclarations (TypeInstanceDeclaration sa ch idx nm deps ofs cls args (ExplicitInstance ds') : rest) =
    (:) <$> (TypeInstanceDeclaration sa ch idx nm deps ofs cls args . ExplicitInstance <$> desugarTypeDeclarations ds')
        <*> desugarTypeDeclarations rest
