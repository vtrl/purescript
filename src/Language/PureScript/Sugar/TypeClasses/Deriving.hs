  ...
  where
          ...
          inst | null reps =
                   ...
                   [ ValueDecl (ss', []) (Ident "to") Public [] $ unguarded $
                      lamCase ss' x
                        [ CaseAlternative
                            [NullBinder]
                            (unguarded (App (Var ss DataGenericRep.to) (Var ss' (Qualified ByNothing x))))
                        ]
                   , ValueDecl (ss', []) (Ident "from") Public [] $ unguarded $
                      lamCase ss' x
                        [ CaseAlternative
                            [NullBinder]
                            (unguarded (App (Var ss DataGenericRep.from) (Var ss' (Qualified ByNothing x))))
                        ]
                   ]
               | otherwise =
                   [ ValueDecl (ss', []) (Ident "to") Public [] $ unguarded $
    ...
    makeInst (DataConstructorDeclaration _ ctorName args) = do
        ...
        return ( srcTypeApp (srcTypeApp (srcTypeConstructor DataGenericRep.Constructor)
               ...
               , CaseAlternative [ ConstructorBinder ss DataGenericRep.Constructor [matchProduct] ]
                                 (unguarded (foldl' App (Constructor ss (Qualified (ByModule mn) ctorName)) ctorArgs))
               , CaseAlternative [ ConstructorBinder ss (Qualified (ByModule mn) ctorName) matchCtor ]
    ...
    makeArg arg = do
      ...
      pure ( srcTypeApp (srcTypeConstructor DataGenericRep.Argument) arg
           ...
           , Var ss (Qualified ByNothing argName)
           , VarBinder ss argName
           , App (Constructor ss DataGenericRep.Argument) (Var ss (Qualified ByNothing argName))
           )
...
deriveEq ss mn syns kinds ds tyConNm = do
  ...
  where
    ...
    preludeConj = App . App (Var ss (Qualified (ByModule (ModuleName "Data.HeytingAlgebra")) (Ident Prelude.conj)))

    preludeEq :: Expr -> Expr -> Expr
    preludeEq = App . App (Var ss (Qualified (ByModule dataEq) (Ident Prelude.eq)))

    preludeEq1 :: Expr -> Expr -> Expr
    preludeEq1 = App . App (Var ss (Qualified (ByModule dataEq) (Ident Prelude.eq1)))
    ...
    mkCtorClause (DataConstructorDeclaration _ ctorName tys) = do
      ...
      let tests = zipWith3 toEqTest (map (Var ss . Qualified ByNothing) identsL) (map (Var ss . Qualified ByNothing) identsR) tys'
      return $ CaseAlternative [caseBinder identsL, caseBinder identsR] (unguarded (conjAll tests))
      where
      caseBinder idents = ConstructorBinder ss (Qualified (ByModule mn) ctorName) (map (VarBinder ss) idents)
...
deriveEq1 ss =
  ...
  where
    preludeEq :: Expr
    preludeEq = Var ss (Qualified (ByModule dataEq) (Ident Prelude.eq))
...
deriveOrd ss mn syns kinds ds tyConNm = do
  ...
  where
    ...
    orderingName = Qualified (ByModule (ModuleName "Data.Ordering")) . ProperName
    ...
    ordCompare = App . App (Var ss (Qualified (ByModule dataOrd) (Ident Prelude.compare)))

    ordCompare1 :: Expr -> Expr -> Expr
    ordCompare1 = App . App (Var ss (Qualified (ByModule dataOrd) (Ident Prelude.compare1)))
    ...
    mkCtorClauses (DataConstructorDeclaration _ ctorName tys, isLast) = do
      ...
      identsR <- replicateM (length tys) (freshIdent "r")
      tys' <- mapM (replaceAllTypeSynonymsM syns kinds . snd) tys
      let tests = zipWith3 toOrdering (map (Var ss . Qualified ByNothing) identsL) (map (Var ss . Qualified ByNothing) identsR) tys'
          extras | not isLast = [ CaseAlternative [ ConstructorBinder ss (Qualified (ByModule mn) ctorName) (replicate (length tys) NullBinder)
                                                  , NullBinder
                                                  ]
                                                  (unguarded (orderingCtor "LT"))
                                , CaseAlternative [ NullBinder
                                                  , ConstructorBinder ss (Qualified (ByModule mn) ctorName) (replicate (length tys) NullBinder)
                                                  ]
...
deriveOrd1 ss =
  ...
  where
    dataOrdCompare :: Expr
    dataOrdCompare = Var ss (Qualified (ByModule dataOrd) (Ident Prelude.compare))
...
mkVarMn ss mn = Var ss . Qualified (fromMaybeModuleName mn)
...
deriveFunctor ss mn syns kinds ds tyConNm = do
  ...
  where
    ...
    mkCtorClause iTyName f (DataConstructorDeclaration _ ctorName ctorTys) = do
      ...
      let ctor = Constructor ss (Qualified (ByModule mn) ctorName)
          rebuilt = foldl' App ctor args
          caseBinder = ConstructorBinder ss (Qualified (ByModule mn) ctorName) (VarBinder ss <$> idents)
