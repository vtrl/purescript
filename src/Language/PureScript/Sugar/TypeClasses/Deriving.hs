...
deriveInstance mn ds decl =
  case decl of
    TypeInstanceDeclaration sa@(ss, _) ch idx nm deps ofs className tys DerivedInstance -> let
      ...
      binaryWildcardClass f = case tys of
        [ty1, ty2] -> case unwrapTypeConstructor ty1 of
          Just (Qualified (ByModuleName mn') tyCon, _, args) | mn == mn' -> do
            ...
            pure $ TypeInstanceDeclaration sa ch idx nm deps ofs className [ty1, ty2'] (ExplicitInstance members)
