...
resolveModuleImport env ie (mn, imps) = foldM go ie imps
  ...
  go ie' (ss, typ, impQual) = do
    modExports <-
      maybe
        (throwError . errorMessage' ss . UnknownName . Qualified ByNothing $ ModName mn)
...
resolveImport importModule exps imps impQual = resolveByType
  ...
  updateImports imps' exps' expName name ss prov =
    let
      ...
      rec = ImportRecord (Qualified (ByModule importModule) name) (exportSourceDefinedIn src) ss prov
    in
      M.alter
        (\currNames -> Just $ rec : fromMaybe [] currNames)
        (Qualified (fromMaybeModuleName impQual) name)
        imps'
