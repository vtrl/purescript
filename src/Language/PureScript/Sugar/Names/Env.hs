...
mkPrimExports ts cs =
  ...
  where
  mkTypeEntry (Qualified (ByModule mn) name) = (name, ([], primExportSource mn))
  mkTypeEntry _ = internalError "mkPrimExports.mkTypEntry: invalid argument"

  mkClassEntry (Qualified (ByModule mn) name) = (name, primExportSource mn)
...
throwExportConflict' ss new existing newName existingName =
  throwError . errorMessage' ss $
    ExportConflict (Qualified (ByModule new) newName) (Qualified (ByModule existing) existingName)
...
checkImportConflicts ss currentModule toName xs =
  ...
  in
    ...
    then case nonImplicit of
      [ImportRecord (Qualified (ByModule mnNew) _) mnOrig _ _] -> do
      ...
      let ImportRecord (Qualified (ByModule mnNew) _) mnOrig _ _ = head byOrig
