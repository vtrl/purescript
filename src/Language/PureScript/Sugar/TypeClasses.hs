...
desugarModule (Module ss coms name decls (Just exps)) = do
  ...
  classDeclName (TypeClassDeclaration _ pn _ _ _ _) = Qualified (ByModule name) pn
...
desugarDecl mn exps = go
  ...
  isExported
    ...
    -> Bool
  isExported test (Qualified (ByModule mn') pn) = mn /= mn' || test pn exps
...
typeClassMemberToDictionaryAccessor mn name args (TypeDeclaration (TypeDeclarationData sa@(ss, _) ident ty)) =
  let className = Qualified (ByModule mn) name
      ...
      acsr = Accessor (mkString $ runIdent ident) (Var ss (Qualified ByNothing dictObjIdent))
  in ValueDecl sa ident Private []
    [MkUnguarded (
     TypedValue False (Abs (VarBinder ss dictIdent) (Case [Var ss $ Qualified ByNothing dictIdent] [CaseAlternative [ctor] [MkUnguarded acsr]])) $
