...
renameInModule imports (Module modSS coms mn decls exps) =
  ...
  updateValue (_, bound) (Var ss name'@(Qualified _ ident)) | ident `notElem` bound =
    ((ss, bound), ) <$> (Var ss <$> updateValueName name' ss)
  updateValue (_, bound) (Var ss name'@(Qualified (ByModule _) _)) =
    ((ss, bound), ) <$> (Var ss <$> updateValueName name' ss)
  ...
  update imps toName qname@(Qualified qb name) pos = warnAndRethrowWithPosition pos $
    case (M.lookup qname imps, qb) of
      ...
      (Just options, _) -> do
        ...
        return $ Qualified (ByModule mnOrig) name
      ...
      (Nothing, ByModule mn'') ->
        ...
        then throwUnknown
        else throwError . errorMessage . UnknownName . Qualified ByNothing $ ModName mn''
