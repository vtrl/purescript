...
desugarCases = desugarRest <=< fmap join . flip parU toDecls . groupBy inSameGroup
  where
    ...
    desugarRest (TypeInstanceDeclaration sa cd idx name constraints ofs className tys ds : rest) =
      (:) <$> (TypeInstanceDeclaration sa cd idx name constraints ofs className tys <$> traverseTypeInstanceBody desugarCases ds) <*> desugarRest rest
