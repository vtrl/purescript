...
moduleToCoreFn env (A.Module modSS coms mn decls (Just exps)) =
  ...
  declToCoreFn (A.DataDeclaration (ss, com) Newtype _ _ [ctor]) =
    [NonRec (ss, [], Nothing, declMeta) (properToIdent $ A.dataCtorName ctor) $
      Abs (ss, com, Nothing, Just IsNewtype) (Ident "x") (Var (ssAnn ss) $ Qualified ByNothing (Ident "x"))]
    where
  ...
  declToCoreFn (A.DataDeclaration (ss, com) Data tyName _ ctors) =
    flip fmap ctors $ \ctorDecl ->
      let
        ...
        (_, _, _, fields) = lookupConstructor env (Qualified (ByModule mn) ctor)
  ...
  exprToCoreFn ss com ty (A.Unused _) =
    Var (ss, com, ty, Nothing) (Qualified (ByModule C.Prim) (Ident C.undefined))
  ...
  getConstructorMeta ctor =
    ...
    typeConstructor (Qualified (ByModule mn') _, (_, tyCtor, _, _)) = (mn', tyCtor)
