...
everywhereOnValues f g h = (f', g', h')
  ...
  f' (TypeInstanceDeclaration sa ch idx name cs ofs className args ds) = f (TypeInstanceDeclaration sa ch idx name cs ofs className args (mapTypeInstanceBody (fmap f') ds))
  f' other = f other
...
everywhereOnValuesTopDownM f g h = (f' <=< f, g' <=< g, h' <=< h)
  ...
  f' (TypeInstanceDeclaration sa ch idx name cs ofs className args ds) = TypeInstanceDeclaration sa ch idx name cs ofs className args <$> traverseTypeInstanceBody (traverse (f' <=< f)) ds
...
everywhereOnValuesM f g h = (f', g', h')
  ...
  f' (TypeInstanceDeclaration sa ch idx name cs ofs className args ds) = (TypeInstanceDeclaration sa ch idx name cs ofs className args <$> traverseTypeInstanceBody (traverse f') ds) >>= f
  f' other = f other
...
everywhereWithContextOnValuesM s0 f g h i j k = (f'' s0, g'' s0, h'' s0, i'' s0, j'' s0, k'' s0)
  ...
  f' s (TypeInstanceDeclaration sa ch idx name cs ofs className args ds) = TypeInstanceDeclaration sa ch idx name cs ofs className args <$> traverseTypeInstanceBody (traverse (f'' s)) ds
...
accumTypes f = everythingOnValues mappend forDecls forValues forBinders (const mempty) (const mempty)
  ...
  forDecls (TypeInstanceDeclaration _ _ _ _ cs _ _ tys _) =
