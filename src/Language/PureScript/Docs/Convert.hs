...
insertValueTypesAndAdjustKinds env m =
  ...
  insertInferredRoles d@Declaration { declInfo = DataDeclaration dataDeclType args [] } =
    ...
    inferredRoles = do
      let key = P.Qualified (P.ByModule (modName m)) (P.ProperName (declTitle d))
  ...
  lookupName name =
    let key = P.Qualified (P.ByModule (modName m)) name
  ...
  insertInferredKind d name keyword =
    let
      key = P.Qualified (P.ByModule (modName m)) (P.ProperName name)
