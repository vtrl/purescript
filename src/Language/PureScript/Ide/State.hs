  ...
  where
    extractInstances mn P.EDInstance{..} =
      case edInstanceClassName of
          P.Qualified (P.ByModule classModule) className ->
            Just (IdeInstance mn
                  edInstanceName
                  edInstanceTypes
...
resolveOperatorsForModule modules = map (idaDeclaration %~ resolveOperator)
  where
    ...
    resolveOperator (IdeDeclValueOperator op)
      | (P.Qualified (P.ByModule mn) (Left ident)) <- op ^. ideValueOpAlias =
      ...
      | (P.Qualified (P.ByModule mn) (Right dtor)) <- op ^. ideValueOpAlias =
          let t = getDeclarations mn
    ...
    resolveOperator (IdeDeclTypeOperator op)
      | P.Qualified (P.ByModule mn) properName <- op ^. ideTypeOpAlias =
