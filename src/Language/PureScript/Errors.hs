...
prettyPrintSingleError (PPEOptions codeColor full level showDocs relPath) e = flip evalState defaultUnknownMap $ do
  ...
  prettyPrintErrorMessage typeMap (ErrorMessage hints simple) =
    ...
    renderSimpleErrorMessage (UnknownName name@(Qualified ByNothing (IdentName (Ident i)))) | i `elem` [ C.bind, C.discard ] =
    ...
    renderSimpleErrorMessage (UnknownName name@(Qualified ByNothing (IdentName (Ident i)))) | i == C.negate =
    ...
    renderSimpleErrorMessage (UnknownImport mn name) =
      paras [ line $ "Cannot import " <> printName (Qualified ByNothing name) <> " from module " <> markCode (runModuleName mn)
    ...
    renderSimpleErrorMessage (UnknownExport name) =
      line $ "Cannot export unknown " <> printName (Qualified ByNothing name)
    ...
    renderSimpleErrorMessage (ScopeConflict nm ms) =
      paras [ line $ "Conflicting definitions are in scope for " <> printName (Qualified ByNothing nm) <> " from the following modules:"
            , indent $ paras $ map (line . markCode . runModuleName) ms
            ]
    renderSimpleErrorMessage (ScopeShadowing nm exmn ms) =
      paras [ line $ "Shadowed definitions are in scope for " <> printName (Qualified ByNothing nm) <> " from the following open imports:"
    ...
    renderSimpleErrorMessage (DeclConflict new existing) =
      line $ "Declaration for " <> printName (Qualified ByNothing new) <> " conflicts with an existing " <> nameType existing <> " of the same name."
    ...
    renderSimpleErrorMessage msg@(UnusedExplicitImport mn names _ _) =
      paras [ line $ "The import of module " <> markCode (runModuleName mn) <> " contains the following unused references:"
            , indent $ paras $ map (line . markCode . runName . Qualified ByNothing) names
    ...
    renderSimpleErrorMessage (DuplicateImportRef name) =
      line $ "Import list contains multiple references to " <> printName (Qualified ByNothing name)

    renderSimpleErrorMessage (DuplicateExportRef name) =
      line $ "Export list contains multiple references to " <> printName (Qualified ByNothing name)
  ...
  prettyInstanceName = \case
    ...
    Qualified qb (Right inst) -> line . markCode . showQualified showIdent $ Qualified qb inst
