...
createTemporaryModule exec st val =
  let
    ...
    eval          = P.Var internalSpan (P.Qualified (P.ByModule (P.ModuleName "$Support")) (snd (psciInteractivePrint st)))
    mainValue     = P.App eval (P.Var internalSpan (P.Qualified P.ByNothing (P.Ident "it")))
                      ...
                      (P.TypeDeclarationData (internalSpan, []) (P.Ident "$main")
                        (P.srcTypeApp
                          (P.srcTypeConstructor
                            (P.Qualified (P.ByModule (P.ModuleName "$Effect")) (P.ProperName "Effect")))
                                  P.srcTypeWildcard))
