...
applyExternsFileToEnvironment ExternsFile{..} = flip (foldl' applyDecl) efDeclarations
  ...
  applyDecl env (EDValue ident ty) = env { names = M.insert (Qualified (ByModule efModuleName) ident) (ty, External, Defined) (names env) }
  ...
  qual :: a -> Qualified a
  qual = Qualified (ByModule efModuleName)
...
moduleToExternsFile (Module ss _ mn ds (Just exps)) env renamedIdents = ExternsFile{..}
  ...
  toExternsDeclaration (TypeRef _ pn dctors) =
    case Qualified (ByModule mn) pn `M.lookup` types env of
      Nothing -> internalError "toExternsDeclaration: no kind in toExternsDeclaration"
      Just (kind, TypeSynonym)
        | Just (args, synTy) <- Qualified (ByModule mn) pn `M.lookup` typeSynonyms env -> [ EDType pn kind TypeSynonym, EDTypeSynonym pn args synTy ]
      ...
      Just (kind, tk@(DataType _ _ tys)) ->
        EDType pn kind tk : [ EDDataConstructor dctor dty pn ty args
                            ...
                            , (dty, _, ty, args) <- maybeToList (Qualified (ByModule mn) dctor `M.lookup` dataConstructors env)
                            ]
      _ -> internalError "toExternsDeclaration: Invalid input"
  toExternsDeclaration (ValueRef _ ident)
    | Just (ty, _, _) <- Qualified (ByModule mn) ident `M.lookup` names env
  ...
  toExternsDeclaration (TypeClassRef _ className)
    ...
    , Just TypeClassData{..} <- Qualified (ByModule mn) className `M.lookup` typeClasses env
    , Just (kind, tk) <- Qualified (ByModule mn) (coerceProperName className) `M.lookup` types env
    , Just (dictKind, dictData@(DataType _ _ [(dctor, _)])) <- Qualified (ByModule mn) dictName `M.lookup` types env
    , Just (dty, _, ty, args) <- Qualified (ByModule mn) dctor `M.lookup` dataConstructors env
  ...
  toExternsDeclaration (TypeInstanceRef ss' ident ns)
    = [ EDInstance tcdClassName (lookupRenamedIdent ident) tcdForAll tcdInstanceKinds tcdInstanceTypes tcdDependencies tcdChain tcdIndex ns ss'
      ...
      , m2 <- M.elems m1
      , nel <- maybeToList (M.lookup (Qualified (ByModule mn) ident) m2)
