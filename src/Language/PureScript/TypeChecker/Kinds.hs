...
inferKind = \tyToInfer ->
  ...
  go = \case
    ...
    ty@(TypeVar ann v) -> do
      ...
      kind <- apply =<< lookupTypeVariable moduleName (Qualified ByNothing $ ProperName v)
      pure (ty, kind $> ann)
...
elaborateKind = \case
  ...
  TypeVar ann a -> do
    moduleName <- unsafeCheckCurrentModule
    kind <- apply =<< lookupTypeVariable moduleName (Qualified ByNothing $ ProperName a)
    pure (kind $> ann)
...
inferDataDeclaration moduleName (ann, tyName, tyArgs, ctors) = do
  tyKind <- apply =<< lookupTypeVariable moduleName (Qualified ByNothing tyName)
...
inferClassDeclaration moduleName (ann, clsName, clsArgs, superClasses, decls) = do
  clsKind <- apply =<< lookupTypeVariable moduleName (Qualified ByNothing $ coerceProperName clsName)
...
existingSignatureOrFreshKind moduleName ss name = do
  env <- getEnv
  case M.lookup (Qualified (ByModule moduleName) name) (E.types env) of
