...
withScopedTypeVars mn ks ma = do
  orig <- get
  forM_ ks $ \(name, _) ->
    when (Qualified (ByModule mn) (ProperName name) `M.member` types (checkEnv orig)) $
      tell . errorMessage $ ShadowedTypeVar name
  bindTypes (M.fromList (map (\(name, k) -> (Qualified (ByModule mn) (ProperName name), (k, ScopedTypeVar))) ks)) ma
...
withTypeClassDictionaries entries action = do
  ...
  let mentries =
        M.fromListWith (M.unionWith (M.unionWith (<>)))
          [ (getQual value, M.singleton className (M.singleton value (pure entry)))
          | entry@TypeClassDictionaryInScope{ tcdValue = value, tcdClassName = className }
              <- entries
          ]
...
bindLocalVariables bindings =
  bindNames (M.fromList $ flip map bindings $ \(name, ty, visibility) -> (Qualified ByNothing name, (ty, Private, visibility)))
...
bindLocalTypeVariables moduleName bindings =
  bindTypes (M.fromList $ flip map bindings $ \(pn, kind) -> (Qualified (ByModule moduleName) pn, (kind, LocalTypeVariable)))
...
lookupTypeVariable
  ...
  -> m SourceType
lookupTypeVariable currentModule (Qualified qualifiedBy name) = do
  env <- getEnv
  case M.lookup (Qualified qualifiedBy' name) (types env) of
...
getLocalContext = do
  env <- getEnv
  return [ (ident, ty')
         | (Qualified qb ident@Ident{}, (ty', _, Defined)) <- M.toList (names env)
         , isByNothing qb || isBySourceSpan qb
         ]
