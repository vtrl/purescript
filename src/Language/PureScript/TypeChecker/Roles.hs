...
inferDataBindingGroupRoles env moduleName roleDeclarations group =
  let declaredRoleEnv = M.fromList $ map (Qualified (ByModule moduleName) . rdeclIdent &&& rdeclRoles) roleDeclarations
  ...
  in \tyName tyArgs ->
        let qualTyName = Qualified (ByModule moduleName) tyName
