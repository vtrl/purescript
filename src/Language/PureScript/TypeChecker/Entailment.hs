...
entails SolverOptions{..} constraint context hints =
  ...
  where
    ...
    forClassName _ ctx cn@(Qualified (ByModule mn) _) _ tys = concatMap (findDicts ctx cn) (ordNub (Nothing : Just mn : map Just (mapMaybe ctorModules tys)))
    forClassName _ _ _ _ _ = internalError "forClassName: expected qualified class name"

    ctorModules :: SourceType -> Maybe ModuleName
    ctorModules (TypeConstructor _ (Qualified (ByModule mn) _)) = Just mn
    ctorModules (TypeConstructor _ (Qualified _ _)) = internalError "ctorModules: unqualified type name"
    ...
    valUndefined :: Expr
    valUndefined = Var nullSourceSpan (Qualified (ByModule C.Prim) (Ident C.undefined))
        ...
        go work hints' con@(Constraint _ className' kinds' tys' conInfo) = WriterT . StateT . (withErrorMessageHint (ErrorSolvingConstraint con) .) . runStateT . runWriterT $ do
            ...
            case solution of
              ...
              Unsolved unsolved -> do
                ...
                let qident = Qualified ByNothing ident
          ...
          where
            ...
            tcdToInstanceDescription TypeClassDictionaryInScope{ tcdDescription, tcdValue } =
              ...
              in case tcdDescription of
                Just ty -> flip Qualified (Left ty) <$> fmap (fromMaybeModuleName . getQual) nii
