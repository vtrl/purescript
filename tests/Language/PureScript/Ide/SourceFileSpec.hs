...
valueFixity =
  P.ValueFixityDeclaration
    ann1
    (P.Fixity P.Infix 0)
    (P.Qualified P.ByNothing (Left (P.Ident "")))
    (P.OpName "<$>")
typeFixity =
  P.TypeFixityDeclaration
    ann1
    (P.Fixity P.Infix 0)
    (P.Qualified P.ByNothing (P.ProperName ""))
    (P.OpName "~>")
  ...
  where
    ideState = emptyIdeState `volatileState`
      [ ("Test",
         ...
         , ideValueOp "<$>" (P.Qualified P.ByNothing (Left "")) 0 Nothing Nothing
           `annLoc` valueOpSS
         , ideTypeOp "~>" (P.Qualified P.ByNothing "") 0 Nothing Nothing
           `annLoc` typeOpSS
         ])
      ]
