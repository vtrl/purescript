  ...
  context "Expr" $ do
    ...
    specify "should parse Abs" $ do
      let m = Module ss [] mn mp [] [] M.empty []
                [ NonRec ann (Ident "abs")
                    $ Abs ann (Ident "x") (Var ann (Qualified (ByModule mn) (Ident "x")))
                ]
      parseMod m `shouldSatisfy` isSuccess
    ...
    specify "should parse App" $ do
      let m = Module ss [] mn mp [] [] M.empty []
                [ NonRec ann (Ident "app")
                    $ App ann
                        (Abs ann (Ident "x") (Var ann (Qualified ByNothing (Ident "x"))))
                        (Literal ann (CharLiteral 'c'))
                ]
      parseMod m `shouldSatisfy` isSuccess

    specify "should parse UnusedIdent in Abs" $ do
      let i = NonRec ann (Ident "f") (Abs ann UnusedIdent (Var ann (Qualified ByNothing (Ident "x"))))
    ...
    specify "should parse Case" $ do
      let m = Module ss [] mn mp [] [] M.empty []
                [ NonRec ann (Ident "case") $
                    Case ann [Var ann (Qualified ByNothing (Ident "x"))]
                      [ CaseAlternative
                        [ NullBinder ann ]
    ...
    specify "should parse Let" $ do
      let m = Module ss [] mn mp [] [] M.empty []
                [ NonRec ann (Ident "case") $
                    Let ann
                      [ Rec [((ann, Ident "a"), Var ann (Qualified ByNothing (Ident "x")))] ]
                ...
                [ NonRec ann (Ident "case") $
                    Case ann [Var ann (Qualified ByNothing (Ident "x"))]
                      [ CaseAlternative
                        [ ConstructorBinder
                            ann
                            (Qualified (ByModule (ModuleName "Data.Either")) (ProperName "Either"))
                            (Qualified ByNothing (ProperName "Left"))
